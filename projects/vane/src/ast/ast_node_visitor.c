...
void ast_node_visit_with(ASTNode* parent, ASTNode* node, const ASTVisitor* visitor) {
    ...
    if (visitor->pre_fn != NULL) {
        visitor->pre_fn(parent, node, visitor->ctx);
    }

    switch (node->kind) {
#define AST_NODE_VISIT_CHILD_FN(CHILD, ...) ast_node_visit_with(node, CHILD, visitor);
...
#include "vane/ast/ast_node_kind.def"
    ...
    if (visitor->post_fn != NULL) {
        visitor->post_fn(parent, node, visitor->ctx);
    }
}